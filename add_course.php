    ...
    if (empty($subject_id) || empty($name) || empty($description)) {
        ...
        if ($stmt->execute()) {
            ...
            if (isset($_POST['case_study_names']) && isset($_POST['case_study_contents']) && isset($_POST['case_study_correction_names']) && isset($_POST['case_study_correction_contents'])) {
                ...
                $count = min(count($case_study_names), count($case_study_contents), count($case_study_correction_names), count($case_study_correction_contents));
                ...
                for ($i = 0; $i < $count; $i++) {
                    if (!empty($case_study_names[$i]) && !empty($case_study_contents[$i]) && !empty($case_study_correction_names[$i]) && !empty($case_study_correction_contents[$i])) {
                        ...
                        $stmt->bind_param('issss', $course_id, $case_study_names[$i], $case_study_correction_names[$i], $case_study_contents[$i], $case_study_correction_contents[$i]);
                        $stmt->execute();
                    }
                }
            }
